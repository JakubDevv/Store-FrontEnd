...
export class DiscountedComponent implements OnInit {
  ...
  constructor(public router: Router, public route: ActivatedRoute, private productsService: ProductsService) { }

  ngOnInit(): void {
    let queryParam = this.route.snapshot.queryParamMap.get("page") ?? "1";
    this.pageNumber = parseInt(queryParam);
    this.productsService.getDiscountedProducts(parseInt(queryParam)).subscribe((data:any)=>{
  ...
  goToNextPage(number: number) {
    ...
    if(number<=this.totalPages && number > 0)
      this.router.navigate(['/discounted'], navigationExtras).then(()=> { window.location.reload() });
  }
